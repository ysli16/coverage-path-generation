...
for i in vpoints:
    ...
    num.append(len(i))
'''
def addtomodel(newcell,width,cells,vpoints,innercost):
    vpoints.append([])
    innercost.append([])
    index=len(vpoints)-1
    ...
    for i in range(len(vpoints[-1])):
        for j in range(i):
            optimalpath=singlearearoute(newcell,width,vpoints[-1][i],vpoints[-1][j])
...
for cell in convexcells:
    addtomodel(cell,width[i],convexcells,vpoints,innercost)
    i=i+1
...
if(findcno(current)==findcno(next1)):
    ...
    cellwaypoint=singlearearoute(convexcells[cellno],width[current],vpoints[cellno][verticeno1],vpoints[cellno][verticeno2])
