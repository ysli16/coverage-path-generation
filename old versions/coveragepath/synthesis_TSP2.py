        ...
        if not(len(crossline)==0):
            for line in crossline:
                if(line.contains(Point(pt))):
                    ...
                    cells.append(cell)
                    break
addtomodel(remain,points)
cells.append(remain)
for i in range(obsnum):
    if(iscell[i]):
        addtomodel(obstacles[i],points)
...
current=1
visited=[current]
cellwaypoint=singlearearoute(cells[current-1],inputwidth,points[(current-1)*2],points[(current-1)*2+1])
...
while(len(visited)<len(cells)):      
    ...
    if(no1==current):
        ...
        if(nextvar[1]%2==1):
            ...
            cellwaypoint=singlearearoute(cells[current-1],width,waypoints[-1],points[(current-1)*2])
            #cellwaypoint.reverse()
        else:
            #cellwaypoint=singlearearoute(cells[current-1],width,points[(current-1)*2],points[(current-1)*2+1])
            cellwaypoint=singlearearoute(cells[current-1],width,waypoints[-1],points[(current-1)*2+1])
    else:
        current=no1
        if(current==len(cells)):
            width=0.4
