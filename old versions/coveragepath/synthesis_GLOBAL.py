...
for i in vpoints:
    ...
    num.append(len(i))
'''
def addtomodel(newcell,cells,vpoints,innercost):
    vpoints.append([])
    innercost.append([])
    index=len(vpoints)-1
    ...
    for i in range(len(vpoints[-1])):
        for j in range(i):
            optimalpath=singlearearoute(newcell,inputwidth,vpoints[-1][i],vpoints[-1][j])
            cost[i][j]=timeconsume(optimalpath,vpoints[-1][i],vpoints[-1][j])
            cost[j][i]=cost[i][j]
...
def findnext(pathvar,visited,current):
    for i in range(totalnum):
        if not(pathvar[(i,current)].X==0):
            if not(isvisited(visited,i)):
                return i
...
for cell in cells:
    addtomodel(cell,cells,vpoints,innercost)
...
if(findcno(current)==findcno(next1)):
    ...
    cellwaypoint=singlearearoute(cells[cellno],inputwidth,vpoints[cellno][verticeno1],vpoints[cellno][verticeno2])
    plotpolygon(ax,cells[cellno])
...
while(next1):    
    if(findcno(current)==findcno(next1)):
        ...
        verticeno1=findvno(cellno,current)
        verticeno2=findvno(cellno,next1)
        if(cellno==len(cells)-1):
            width=0.4
        else:
            width=inputwidth
        cellwaypoint=singlearearoute(cells[cellno],width,vpoints[cellno][verticeno1],vpoints[cellno][verticeno2])
        plotpolygon(ax,cells[cellno])
        waypoints.extend(cellwaypoint)
    current=next1
    visited.append(current)
