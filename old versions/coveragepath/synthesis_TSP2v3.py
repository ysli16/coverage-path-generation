...
features:
    ...
    consider left and right extremities of each cell as nodes
    assume all nodes are connected
    consider Euclidean distance as cost of each edge
    edges connect the nodes from the same cell must be selected
            ...
            if(p[0]<p[1]):
                ...
                if(no1==current):
                    if(len(visited)==1):
                        ...
                        if not(no2 in visited):
                            return p
                elif(no2==current):
                    if not(no1 in visited):
                        return p
